...
pub(crate) trait Output: Sized {
    ...
    fn try_open(
        spec: SignalSpec,
        duration: Duration,
    ) -> std::result::Result<Self, AudioOutputError>;
...
mod output {
    ...
    impl Output for AudioOutput {
        ...
        fn try_open(
            ...
            if !pa_spec.is_valid() {
                return Err(AudioOutputError::InvalidSpec(anyhow!(
                    "invalid stream specification: {pa_spec:#?}"
                )));
            }
            ...
            if pa_ch_map.is_none() {
                return Err(AudioOutputError::Channel(anyhow!(
                    "invalid channels: {:#?}",
                    spec.channels
                )));
            }
            ...
            match pa_result {
                ...
                Err(err) => Err(AudioOutputError::OpenStream(anyhow!(
                    "stream open error: {err}"
                ))),
            }
        }
        ...
        fn write(
        ...
        ) -> std::result::Result<(), AudioOutputError> {
            ...
            self.sample_buf
                .copy_interleaved_ref(self.audio_buf.as_audio_buffer_ref());
    ...
    impl Output for AudioOutput {
        ...
        fn try_open(
            ...
            let device = match host.default_output_device() {
                Some(device) => device,
                _ => {
                    return Err(AudioOutputError::OpenStream(anyhow!(

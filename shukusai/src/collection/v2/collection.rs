...
impl Collection {
    //-------------------------------------------------- Converts v1 from disk into current.
...
mod tests {
    ...
    const C1: Lazy<Collection> = Lazy::new(|| {
        Collection::from_path("../assets/shukusai/state/collection2_new.bin").unwrap()
    });
    // Filled, user `Collection`.
    const C2: Lazy<Collection> = Lazy::new(|| {
        Collection::from_path("../assets/shukusai/state/collection2_real.bin").unwrap()
    });

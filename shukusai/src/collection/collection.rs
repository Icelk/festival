...
use crate::collection::{
    ...
    song::Song,
    AlbumKey, ArtistKey, Key, Map, SongKey,
};
...
pub(crate) static DUMMY_COLLECTION: Lazy<Arc<Collection>> =
    Lazy::new(|| Arc::new(Collection::new()));
...
impl Collection {
    ...
    pub fn song_tail<K: Into<SongKey>>(
        &self,
        key: K,
    ) -> std::iter::Peekable<std::slice::Iter<'_, SongKey>> {
        let key = key.into();
    ...
    pub fn next_album<K: Into<AlbumKey>>(&self, key: K) -> AlbumKey {
        ...
        let index = other_albums.iter().position(|i| i == key).unwrap_or(0);
    ...
    pub fn next_song<K: Into<SongKey>>(&self, key: K) -> SongKey {
        ...
        let index = other_songs.iter().position(|i| i == key).unwrap_or(0);
    ...
    pub fn rand_albums(&self) -> Option<Box<[AlbumKey]>> {
        match self.count_album.usize() {
            ...
            _ => {
                let mut vec: Box<[AlbumKey]> =
                    (0..self.count_album.usize()).map(AlbumKey::from).collect();
                vec.shuffle(&mut rand::thread_rng());
                Some(vec)
            }
        }
    ...
    pub fn rand_songs(&self) -> Option<Box<[SongKey]>> {
        match self.count_song.usize() {
            ...
            _ => {
                let mut vec: Box<[SongKey]> =
                    (0..self.count_song.usize()).map(SongKey::from).collect();
                vec.shuffle(&mut rand::thread_rng());
                Some(vec)
            }
        }
...
"#,
        ...
        while let Some(a) = iter.next() {
            let comma = if iter.peek().is_none() { "\n" } else { "," };
            let track = if let Some(t) = a.track {
                t.to_string()
            } else {
                "null".to_string()
            };
            let disc = if let Some(d) = a.disc {
                d.to_string()
            } else {
                "null".to_string()
            };
...
impl std::fmt::Display for Collection {
    ...
    albums    | {}
    songs     | {}",
            self.empty, self.timestamp, self.count_artist, self.count_album, self.count_song,
        )
    }
}
...
mod tests {
    ...
    const C1: Lazy<Collection> = Lazy::new(|| {
        Collection::from_path("../assets/shukusai/state/collection3_new.bin").unwrap()
    });
    // Filled, user `Collection`.
    const C2: Lazy<Collection> = Lazy::new(|| {
        Collection::from_path("../assets/shukusai/state/collection3_real.bin").unwrap()
    });

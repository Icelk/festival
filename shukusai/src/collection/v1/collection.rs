...
mod tests {
    ...
    const C1: Lazy<Collection> = Lazy::new(|| {
        Collection::from_path("../assets/shukusai/state/collection1_new.bin").unwrap()
    });
    // Filled, user `Collection`.
    const C2: Lazy<Collection> = Lazy::new(|| {
        Collection::from_path("../assets/shukusai/state/collection1_real.bin").unwrap()
    });

...
use shukusai::{
    audio::{Repeat, Volume},
    collection::{AlbumKey, ArtistKey, Collection, Keychain},
...
mod test {
    ...
    // Empty.
    const S1: Lazy<State> =
        Lazy::new(|| State::from_path("../assets/festival/gui/state/state1_new.bin").unwrap());
    // Filled.
    const S2: Lazy<State> =
        Lazy::new(|| State::from_path("../assets/festival/gui/state/state1_real.bin").unwrap());

...
use crate::constants::{
    ALBUMS_PER_ROW_MAX, ALBUMS_PER_ROW_MIN, ALBUM_ART_SIZE_MAX, ALBUM_ART_SIZE_MIN, BONE,
    PREVIOUS_THRESHOLD_MAX, PREVIOUS_THRESHOLD_MIN, SLIDER_CIRCLE_ACTIVE, SLIDER_CIRCLE_HOVERED,
    SLIDER_CIRCLE_INACTIVE,
};
use crate::data::{AlbumSizing, WindowTitle};
use crate::text::*;
use benri::{atomic_store, flip};
use egui::containers::scroll_area::ScrollBarVisibility;
use egui::{
    Button, ComboBox, Label, RichText, ScrollArea, SelectableLabel, Sense, Slider, TextStyle,
};
use shukusai::{
    constants::COPYRIGHT,
    search::SearchKind,
        ...
        ScrollArea::vertical()
            ...
            .show_viewport(ui, |ui, _| {
                ...
                ui.scope(|ui| {
                    ...
                    if ui
                        .add_sized([width - 15.0, text], Button::new("Add folder"))
                        .on_hover_text(ADD_FOLDER)
                        .clicked()
                    {
                        self.add_folder();
                    }
                ...
                for i in 0..collection_paths_len {
                    ui.horizontal(|ui| {
                        ...
                        if ui
                            .add_sized(
                                [SIZE, SIZE],
                                Button::new(RichText::new(UI_MINUS).size(SIZE - 5.0)),
                            )
                            .on_hover_text(REMOVE_FOLDER)
                            .clicked()
                        {
                        ...
                        // Show PATH.
                        if ui
                            .add(Label::new(&string).sense(Sense::click()))
                ...
                ui.scope(|ui| {
                    ...
                    if ui
                        .add_sized([width - 15.0, text], Button::new("Reset Collection"))
                ...
                ui.add_sized([width, text], label)
                    .on_hover_text(ALBUM_SORT_ORDER);
                        ...
                        if ui
                            .add_sized(
                                ...
                                SelectableLabel::new(
                                    ...
                                    format!("[{}] Pixels", self.settings.album_pixel_size),
                                ),
                            )
                            .on_hover_text(STATIC_PIXEL_SIZE)
                            .clicked()
                        {
                            self.settings.album_sizing = AlbumSizing::Pixel;
                        }
                        ...
                        if ui
                            .add_sized(
                                ...
                                SelectableLabel::new(
                                    ...
                                    format!("[{}] Albums Per Row", self.settings.albums_per_row),
                                ),
                            )
                            .on_hover_text(ALBUM_PER_ROW)
                            .clicked()
                        {
                            self.settings.album_sizing = AlbumSizing::Row;
                        }
                ...
                let slider = match self.settings.album_sizing {
                    AlbumSizing::Pixel => {
                        ...
                        Slider::new(
                            &mut self.settings.album_pixel_size,
                    ...
                    AlbumSizing::Row => {
                        ...
                        Slider::new(
                            &mut self.settings.albums_per_row,
                            ALBUMS_PER_ROW_MIN..=ALBUMS_PER_ROW_MAX,
                        )
                    }
                };
                ...
                let label = Label::new(
                    RichText::new(format!(
                        "Previous Threshold ({})",
                        self.settings.previous_threshold
                    ))
                    .color(BONE)
                    .text_style(TextStyle::Heading),
                );
                ui.add_sized([width, text], label)
                    .on_hover_text(PREVIOUS_THRESHOLD);
                ...
                ui.group(|ui| {
                    ui.horizontal(|ui| {
                        ...
                        if ui
                            .add_sized(
                                [width, text],
                                SelectableLabel::new(self.settings.empty_autoplay, "Yes"),
                            )
                            .clicked()
                        {
                            flip!(self.settings.empty_autoplay);
                        }
                        ui.separator();
                        if ui
                            .add_sized(
                                [width, text],
                                SelectableLabel::new(!self.settings.empty_autoplay, "No"),
                            )
                            .clicked()
                        {
                            flip!(self.settings.empty_autoplay);
                ...
                ui.group(|ui| {
                    ui.horizontal(|ui| {
                        ...
                        if ui
                            .add_sized(
                                [width, text],
                                SelectableLabel::new(self.settings.restore_state, "Yes"),
                            )
                            .clicked()
                        {
                            flip!(self.settings.restore_state);
                        }
                        ui.separator();
                        if ui
                            .add_sized(
                                [width, text],
                                SelectableLabel::new(!self.settings.restore_state, "No"),
                            )
                            .clicked()
                        {
                            flip!(self.settings.restore_state);
                ...
                let label = Label::new(
                    RichText::new(format!(
                        "Pixels Per Point ({:.1})",
                        self.settings.pixels_per_point
                    ))
                    .color(BONE)
                    .text_style(TextStyle::Heading),
                );
                ui.add_sized([width, text], label)
                    .on_hover_text(PIXELS_PER_POINT);
                ...
                ui.group(|ui| {
                    ui.horizontal(|ui| {
                        let width = (width / 2.0) - 25.0;
                        if ui
                            .add_sized([width, text], Button::new("+"))
                            .on_hover_text(PIXELS_PER_POINT_ADD)
                            .clicked()
                        {
                        ...
                        if ui
                            .add_sized([width, text], Button::new("-"))
                            .on_hover_text(PIXELS_PER_POINT_SUB)
                            .clicked()
                        {
                ...
                //-------------------------------------------------- Version/Copyright
                let label = Label::new(RichText::new("Version").text_style(TextStyle::Heading));
                ui.add_sized([width, text], label);
                ...
                if ui
                    .add_sized(
                        [width, text / 2.0],
                        Label::new(FESTIVAL_SHUKUSAI_COMMIT).sense(Sense::click()),
                    )
                    .clicked()
                {

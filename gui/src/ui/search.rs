//---------------------------------------------------------------------------------------------------- Use
use crate::constants::{GRAY, SEARCH_MAX_LEN};
use crate::data::SearchSort;
use crate::text::{
    SEARCH_BAR, SEARCH_HELP, SEARCH_SORT_ALBUM, SEARCH_SORT_ARTIST, SEARCH_SORT_SONG,
};
use benri::send;
...
use shukusai::kernel::FrontendToKernel;

//---------------------------------------------------------------------------------------------------- Search
impl crate::data::Gui {
    #[inline(always)]
    pub fn show_tab_search(
        &mut self,
        ui: &mut egui::Ui,
        ctx: &egui::Context,
        width: f32,
        ...
        CentralPanel::default().show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                let response = ui
                    .add_sized([width, 35.0], text_edit)
                    .on_hover_text(SEARCH_BAR);
                ...
                if response.changed() || self.search_jump {
                    ...
                    if self.state.search_string.len() > SEARCH_MAX_LEN {
                        ...
                        send!(
                            self.to_kernel,
                            FrontendToKernel::Search((
                                self.state.search_string.clone(),
                                self.settings.search_kind
                            ))
                        );
                        self.searching = true;
                    }
            ...
            if self.state.search_string.is_empty() {
                ...
                ui.add_sized(
                    [width, hhh],
                    Label::new(RichText::new(&self.count_artist).color(GRAY)),
                );
                ui.add_sized(
                    [width, hhh],
                    Label::new(RichText::new(&self.count_album).color(GRAY)),
                );
                ui.add_sized(
                    [width, hhh],
                    Label::new(RichText::new(&self.count_song).color(GRAY)),
                );
            ...
            ui.group(|ui| {
                ui.horizontal(|ui| {
...

                    {
                        ...
                        let label =
                            SelectableLabel::new(self.settings.search_sort == SORT, SORT.human());
                        if ui
                            .add_sized([width, 30.0], label)
                            .on_hover_text(SEARCH_SORT_SONG)
                            .clicked()
                        {
...

                    {
                        ...
                        if ui
                            .add_sized([width, 30.0], label)
                            .on_hover_text(SEARCH_SORT_ALBUM)
                            .clicked()
                        {
                            self.settings.search_sort = SORT;
                        }
...

                    {
                        ...
                        if ui
                            .add_sized([width, 30.0], label)
                            .on_hover_text(SEARCH_SORT_ARTIST)
                            .clicked()
                        {
                            self.settings.search_sort = SORT;
                        }
                                    ...
                                    .body(|mut body| {
                                        for key in self.state.search_result.songs.iter() {
                                            body.row(ROW_HEIGHT, |mut row| {
                                                ...
                                                match song.track {
                                                    Some(t) => row.col(|ui| {
                                                        ui.label(Unsigned::from(t).as_str());
                                                    }),
                                                    None => row.col(|ui| {
                                                        ui.label("???");
                                                    }),
                                                ...
                                                match song.disc {
                                                    Some(d) => row.col(|ui| {
                                                        ui.label(Unsigned::from(d).as_str());
                                                    }),
                                                    None => row.col(|ui| {
                                                        ui.label("???");
                                                    }),
                                    ...
                                    .body(|mut body| {
                                        for key in self.state.search_result.albums.iter() {
                                            body.row(130.0, |mut row| {
                                                ...
                                                let (artist, _) =
                                                    self.collection.artist_from_album(key);

                                                row.col(|ui| {
                                                    crate::no_rounding!(ui);
                                                    crate::album_button!(
                                                        self, album, *key, ui, ctx, 120.0, ""
                                                    );
                                    ...
                                    .body(|mut body| {
                                        for key in self.state.search_result.artists.iter() {
                                            body.row(130.0, |mut row| {
                                                ...
                                                row.col(|ui| {
                                                    ui.label(artist.runtime.as_str());
                                                });
